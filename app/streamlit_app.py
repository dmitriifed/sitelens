...
BRAND_TITLE_HTML = (
    "<h1 style='margin:0 0 0.25rem 0; font-weight:700; line-height:1.1;'>"
    "<span style='color:#FAFAFA;'>Site</span>"
    "<span style='color:#A41E1E;'>Lens</span>"
...
def load_polygons() -> dict:
    """s_fid -> GeoJSON geometry dict for building footprints."""
    import geopandas as gpd
    path = REPO_ROOT / "data" / "noto_crops" / "polygons.parquet"
    gdf = gpd.read_parquet(path)
    return {row.s_fid: row.geometry.__geo_interface__ for row in gdf.itertuples()}
...
def build_map(hits, selected_id=None, map_center=None, map_zoom=None):
    ...
    if map_center is not None and map_zoom is not None:
        ...
        if len(geo_hits) == 1:
            ...
            if   spread > 0.10:  zoom = 13  # > 10 km — multi-municipality
            elif spread > 0.05:  zoom = 14  # 5–10 km
            elif spread > 0.02:  zoom = 15  # 2–5 km
            elif spread > 0.01:  zoom = 16  # 1–2 km
            elif spread > 0.005: zoom = 17  # 500 m–1 km
            else:                zoom = 18  # tight cluster
    else:
    ...
    for h in geo_hits:
        ...
        if geom:
            folium.GeoJson(
                ...
                style_function=lambda _, c=color, sel=is_selected: {
                    "color": "white" if sel else c,
                    "fillColor": c,
                    "weight": 3 if sel else 1.5,
                    "fillOpacity": 0.75,
                },
                highlight_function=lambda _: {"weight": 3, "fillOpacity": 0.95},
                popup=popup,
            ).add_to(m)
        else:
            folium.CircleMarker(
...
st.markdown(
    """
<style>
/* Route B: translucent panel floating over the full-width map */
.st-key-sidepanel {
...
# --------------------------------------------------------------------------
# Intro screen — shown once per session; gated on session state so it does
# not reappear after the user enters the demo.  Pinecone and the embed model
# are not initialised until the user clicks through, so the landing paints
# faster and has fewer cold-start failure surfaces.
# --------------------------------------------------------------------------

if not st.session_state.get("entered", False):
    st.markdown(BRAND_TITLE_HTML, unsafe_allow_html=True)
    st.markdown(
        "SiteLens reads post-disaster aerial imagery and returns a per-building "
        "damage assessment a field inspector can act on. It works over the Noto "
        "Peninsula 2024 earthquake and fire zone, pairing Japanese government "
        "orthophotos with peer-reviewed ground-survey damage labels "
        "(Vescovo et al. 2025). Pull up any building, see how it was classified "
        "and which affected buildings sit nearest to it, then generate a written "
        "assessment for the person who has to read it."
    )

    st.subheader("Who it is for")
    st.markdown("\n".join([
        "The same assessment is rewritten for whoever needs to read it:",
        "",
        "- Insurance adjusters, who need a claim-ready damage summary",
        "- Structural engineers, who need the physical read",
        "- Legal counsel, who need a defensible, sourced record",
    ]))

    st.subheader("How to use it")
    st.markdown("\n".join([
        "1. Choose a scenario from the dropdown, type your own description of a "
        "building, or enter a building number from the map.",
        "2. Set how many buildings to return — the target plus its nearest "
        "affected neighbours.",
        "3. Press ASSESS. The map highlights the buildings and the notes column "
        "on the right fills in the assessment.",
        "4. Switch the audience to rewrite the assessment for that reader, and "
        "open the audit panel to see the records behind it.",
    ]))

    st.write("")
    _, _mid, _ = st.columns([1, 1, 1])
    with _mid:
        if st.button("Open the demo", type="primary", use_container_width=True):
...
# Route B: map fills the full width; the panel floats over it as a translucent
# overlay (positioning handled by the .st-key-sidepanel CSS above).
...
with notes_col:
    with st.container(border=False, key="sidepanel"):

        # ---- Heading, instructions, inputs, results: one scrolling panel -----
        ...
        st.caption(
            "Pick a scenario or describe a building, set how many to return, "
            "then press ASSESS."
        )
...
# Drag handle: injected from a zero-height iframe so the script runs in the
# parent document where the panel lives.
st.components.v1.html("""
<script>
(function () {
    function init() {
        var panel = window.parent.document.querySelector('.st-key-sidepanel');
        ...
        handle.innerHTML = '<div style="width:32px;height:4px;border-radius:2px;background:rgba(255,255,255,0.25)"></div>';
